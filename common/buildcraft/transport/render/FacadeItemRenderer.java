...
public class FacadeItemRenderer implements IItemRenderer {
	...
	private void renderFacadeItem(RenderBlocks render, ItemStack item, float translateX, float translateY, float translateZ) {
		...
		render.renderFaceXPos(block, 0.0D, 0.0D, 0.0D, block.getIcon(2, decodedMeta));
		tessellator.draw();
		tessellator.startDrawingQuads();
		tessellator.setNormal(0.0F, 0.0F, 1.0F);
		render.renderFaceXNeg(block, 0.0D, 0.0D, 0.0D, block.getIcon(3, decodedMeta));
		tessellator.draw();
		tessellator.startDrawingQuads();
		tessellator.setNormal(-1F, 0.0F, 0.0F);
		render.renderFaceZNeg(block, 0.0D, 0.0D, 0.0D, block.getIcon(4, decodedMeta));
		tessellator.draw();
		tessellator.startDrawingQuads();
		tessellator.setNormal(1.0F, 0.0F, 0.0F);
		render.renderFaceZPos(block, 0.0D, 0.0D, 0.0D, block.getIcon(5, decodedMeta));
		tessellator.draw();
